    ...
    public function update(Request $request, $id)
    {
        ...
        if($this->setting->where('name',$request->name)->get()->count() > 0){
            if($request->name == $name){
                ...
                return redirect()->route('setting.index')->with('success-message','Data telah dirubah');
            }else{
                return redirect()->route('setting.index')->with('error-message','Gagal Merubah Data '.$request->name.' sudah ada,   edit data yang ada');
            }
        }else{
            $this->setting->find($id)->update($request->all());
            return redirect()->route('setting.index')->with('success-message','Data telah dirubah');
